...
(comment
  (clojure.core.async/put! fallout.core/guesses-chan "SIGMA"))
...
(defn start-game
  ...
  (let [words (->> (words-of-length words length) shuffle (map str/upper-case) (take difficulty))
    ...
    (doseq [w words]
      (println w))
    (println "\nPut your guess on the fallout.core/guesses-chan channel:\n(clojure.core.async/put! guesses-chan \"myguess\")")
